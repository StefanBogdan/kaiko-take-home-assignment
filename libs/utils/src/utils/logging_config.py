...
def log_data_processing_stats(
    logger: logging.Logger, operation: str, records_processed: int, duration: float
):
    """Log data processing statistics."""
    rate = records_processed / duration if duration > 0 else 0
    logger.info(
        f"{operation}: {records_processed} records in {duration:.2f}s "
